...
class FileLogbook:
    ...
    def message_on_start(self, message:str, apparatus:str, run_num:int, run_type:str, user:str):
        self.run_num = run_num
        self.run_type = run_type
        self.website = self.file_name
        import time
        f = open(self.file_name, "a")
        f.write(f"-- User {user} started a run {self.run_num}, of type {self.run_type} on {apparatus} @{self.now()} --\n")
        f.write(f'{user} @{self.now()}: {message}\n')
        f.close()

    def add_message(self, message:str, apparatus:str, user:str):
        f = open(self.file_name, "a")
        f.write(f'{user} @{self.now()}: {message}\n')
        f.close()

    def message_on_stop(self, message:str, apparatus:str, user:str):
        f = open(self.file_name, "a")
        f.write(f"-- User {user} stopped the run {self.run_num}, of type {self.run_type} on {apparatus} @{self.now()}--\n")
        f.write(f'{user} @{self.now()}: {message}\n')
        f.close()
...
class ElisaLogbook:
    ...
    def _send_message(self, subject:str, body:str, command:str, user:str):
    ...
    def message_on_start(self, user:str, message:str, apparatus:str, run_num:int, run_type:str):
        ...
        self._send_message(subject=title, body=text, command='start', user=user)

    def add_message(self, user:str, message:str, apparatus:str):
        if message != "":
            text = f"<p>{user}: {message}</p>"
            self._send_message(subject="User comment", body=text, command='message', user=user)

    def message_on_stop(self, user:str, message:str, apparatus:str):
        ...
        self._send_message(subject=title, body=text, command='stop', user=user)
